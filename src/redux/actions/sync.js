...
export const doGetSyncDesktop = (cb?, password) => (dispatch, getState) => {
  ...
  return getSavedPassword().then(savedPassword => {
    const passwordArgument =
      password || password === '' ? password : savedPassword === null ? '' : savedPassword;
...
export function doSyncSubscribe() {
  return (dispatch, getState) => {
    ...
    if (hasVerifiedEmail && syncEnabled && !syncLocked) {
      dispatch(
        doGetSyncDesktop((error, hasNewData) => dispatch(doHandleSyncComplete(error, hasNewData)))
      );
      ...
      syncTimer = setInterval(() => {
        ...
        if (syncEnabled) {
          dispatch(
            doGetSyncDesktop((error, hasNewData) =>
              dispatch(doHandleSyncComplete(error, hasNewData))
            )
          );
          dispatch(doAnalyticsTagSync());
        }
